...
(
    ...
    //  {{{
    // Pipeline Stage 0
    hpdcache_req_t           st0_req, st0_req_q, st0_req_d;
    ...
    logic                    dir_cache_gnt, dir_cache_gnt_q;
    ...
    hpdcache_coherence_t   write_dir_coherence_wdata, write_dir_coherence_wdata_q, write_dir_coherence_wdata_d;
    hpdcache_dir_addr_t    write_dir_coherence_set, write_dir_coherence_set_q;
    ...
    hpdcache_way_vector_t  coherence_rway, coherence_rway_q;
    logic                  coherence_dir_hit, coherence_dir_hit_q;

    coherence_evict_t coherence_evict_d, coherence_evict_q;
    logic             coherence_replace, coherence_replace_q, coherence_replace_d, replace_ready;
    ...
    logic pending_coherence_rw;

    // hpdcache_dir_entry_t   read_dir_coherence_rdata, coherence_evict_rdata;
    hpdcache_coherence_t   read_dir_coherence_rdata, coherence_evict_rdata, coherence_evict_rdata_q;
    ...
    logic                 refill_busy, refill_busy_q, refill_busy_d;
    logic                 rw_busy, rw_busy_q, rw_busy_d;
    // logic                 coherence_refill_ready, coherence_refill_ready_q;
    logic                 refill_req_ready;
    logic                 abort_inv_write;

    logic replace_read, rsp_read, refill_read, fwd_read, core_req_read;
    logic replace_read_q, rsp_read_q, refill_read_q, fwd_read_q, core_req_read_q;

    // logic [1:0] current_int_offset;
    ...
    `FF(coherence_rway_q, coherence_rway, '0, clk_i, rst_ni)
    `FF(coherence_dir_hit_q, coherence_dir_hit, 1'b0, clk_i, rst_ni)
    // `FF(evict_read_q, evict_read, 1'b0, clk_i, rst_ni)
    // `FF(coherence_refill_ready_q, coherence_refill_ready, 1'b0, clk_i, rst_ni)

    // `FF(replace_read_q, replace_read, 1'b0, clk_i, rst_ni)
    `FF(rsp_read_q, rsp_read, 1'b0, clk_i, rst_ni)
    `FF(refill_read_q, refill_read, 1'b0, clk_i, rst_ni)
    `FF(fwd_read_q, fwd_read, 1'b0, clk_i, rst_ni)
    `FF(core_req_read_q, core_req_read, 1'b0, clk_i, rst_ni)

    `FF(write_dir_coherence_set_q, write_dir_coherence_set, '0, clk_i, rst_ni)
    ...
    hpdcache_ctrl_pe #(
        ...
        .core_req_valid_i,
        // .core_req_ready_o                   (pe_core_req_ready),
        .core_req_ready_o,
        .coherence_req_ready_i              (coherence_core_req_ready),
    ...
    *************************/
    ...
    always_comb begin : refill_busy_comb
        refill_busy_d = refill_busy_q;
        // if (refill_req_valid_i && refill_req_ready_o) begin
        if (refill_write_dir_i && refill_core_rsp_valid_d) begin
            refill_busy_d = 1'b1;
        end
    end

    always_ff @(posedge clk_i or negedge rst_ni) begin : refill_busy_ff
        if (!rst_ni) begin
            refill_busy_q <= 1'b0;
        end else if (refill_busy_d && write_dir_coherence_d && dir_coherence_gnt) begin
            refill_busy_q <= 1'b0;
        end else begin
            refill_busy_q <= refill_busy_d;
        end
    end

    always_comb begin
        rw_busy_d = rw_busy_q;
        if (core_req_valid_i && core_req_ready_o && core_req_is_rw) begin
            rw_busy_d = 1'b1;
        end
        // else if (rw_busy_q && coherence_write_complete) begin
        //     rw_busy_d = 1'b0;
        // end
    end
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            rw_busy_q <= 1'b0;
        // end else if (rw_busy_d && 
        //             (core_rsp_valid_o && st1_req_is_load && !refill_core_rsp_valid_d) ||
        //             (coherence_act_q.hit && st1_req_is_store)) begin
        end else if (rw_busy_q && coherence_write_complete) begin
            rw_busy_q <= 1'b0;
        end else begin
            rw_busy_q <= rw_busy_d;
        end
    ...
    always_comb begin
        ...
        if (rstall || wstall || evict_stall || inv_stall) begin
            ...
            //     coherence_busy_d = 1'b1;
            end else if (fwd_busy) begin
                coherence_busy_d = 1'b1;
            end

        // if (coherence_act_q.hit) begin
        //     coherence_busy_d = 1'b0;
        end else 
        if ((core_req_valid_i && core_req_ready_o && core_req_is_rw) || 
                    fwd_rx_valid_i || 
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            coherence_busy_q <= 1'b0;
                    ...
                    free_coherence_q || coherence_act.stall || abort_inv_write) begin
        // coherence_act.hit || 
    ...
    assign refill_req_ready_o = refill_req_ready && !coherence_busy_d && !fwd_busy_d;
    // assign pending_coherence_rw = read_dir_coherence_q || write_dir_coherence_q;

    logic low_latency_dir_read;

    always_comb begin : coherence_ready
        fwd_rx_ready_o              = 1'b0;
        coherence_rsp_ready_o       = 1'b0;
        coherence_core_req_ready    = 1'b0;
        low_latency_dir_read        = 1'b0;
        replace_ready               = 1'b0;
        // coherence_refill_ready      = 1'b0;
        // Fwd is sink with higher priority than req
        if (!coherence_busy && !write_dir_coherence_d && !fwd_busy_q && !refill_busy_d) begin
            // if (refill_core_rsp_valid_d) begin
            ...
            //     replace_ready = 1'b1;
            // end 
            else if (core_req_valid_i && !core_req_stall) begin
                coherence_core_req_ready    = 1'b1;
            end
    ...
    always_comb begin : pending_core_req
        ...
        if (core_req_valid_i && core_req_ready_o && core_req_is_rw) begin
            // Core req can be held even when handshake is complete
            // Eviction should be handled before processing the request itself
            pending_core_req_d = 1'b1;
        end else if (coherence_op == OP_REPLACE) begin
            pending_core_req_d = 1'b1;
        end
    end

    always_ff @(posedge clk_i or negedge rst_ni) begin : pending_core_req_ff
        if (!rst_ni) begin
            pending_core_req_q <= 1'b0;
        end else if (!core_req_stall && (coherence_op != OP_REPLACE)) begin
            // Req should be resolved once there is no stall, deassert next cycle
            pending_core_req_q <= 1'b0;
        end else begin
            pending_core_req_q <= pending_core_req_d;
        end
    end

    // assign core_req_ready_o = coherence_core_req_ready && pe_core_req_ready;
    ...
    always_comb begin
        ...
        if (refill_core_rsp_valid_i) begin
        // if (refill_write_dir_i && refill_core_rsp_valid_d) begin
            refill_core_rsp_d       = refill_core_rsp_i;
            refill_core_rsp_valid_d = 1'b1;
        end
        
        if (coherence_rsp_valid_i && coherence_rsp_ready_o) begin
            // core_req_d              = core_req_q;
            // core_req_tag_d          = core_req_tag_q;
    ...
    always_comb begin : coherence_req_op_decode
        ...
        // if (st1_req_updt_sel_victim && dir_coherence_gnt_q) begin
        // if (st1_req_updt_sel_victim && coherence_read_served_q && current_state != HPDCACHE_INVALID) begin
        //     coherence_op    = OP_EVICT;
        //     op_decoded      = 1'b1;
        // // end else if (coherence_rsp_valid_d && dir_coherence_gnt_q) begin
        // end else 
        // if (refill_core_rsp_valid_d && coherence_read_served_q) begin
        if (refill_read_q && coherence_read_served_q) begin
        ...
        // end else if (coherence_rsp_valid_d && coherence_read_served_q) begin
        end else if (rsp_read_q && coherence_read_served_q) begin
        ...
        end else if (coherence_replace_q) begin
        // end else if (coherence_replace_q && replace_ready) begin
        // end else if (coherence_replace) begin
            // coherence_op = coherence_busy_q ? OP_REPLACE : OP_EVICT;
            coherence_op = OP_REPLACE;
            op_decoded   = 1'b1;
        // FIXME: CONTIUE HERE
        // end else if (core_req_valid_d && coherence_read_served_q && !core_req_stall) begin
        end else if (core_req_read_q && coherence_read_served_q) begin
        ...
        // Move before req, evict then process req
        end else begin
            coherence_op = OP_NONE;
        end
    end

    assign coherence_replace =  !coherence_dir_hit_q && st1_dir_victim_valid && write_dir_coherence &&
                                (st1_dir_victim_way == write_dir_coherence_way_d) &&
                                write_dir_coherence_wdata.tag != coherence_evict_rdata.tag;
    `FF(coherence_replace_q, coherence_replace, 1'b0, clk_i, rst_ni)
    `FF(coherence_evict_rdata_q, coherence_evict_rdata, '0, clk_i, rst_ni)

    // always_comb begin : replace_decision
    //     coherence_replace_d = coherence_replace_q;

    //     if (st1_dir_victim_valid && write_dir_coherence_d &&
    //         (st1_dir_victim_way == write_dir_coherence_way_d) &&
    //         (write_dir_coherence_wdata.tag != coherence_evict_rdata_q.tag)) begin
    //         coherence_replace_d = 1'b1;
    //     end
    // end

    // always_ff @(posedge clk_i or negedge rst_ni) begin
    //     if (!rst_ni) begin
    //         coherence_replace_q <= 1'b0;
    //     end else if (replace_ready) begin
    //         coherence_replace_q <= 1'b0;
    //     end else begin
    //         coherence_replace_q <= coherence_replace_d;
    //     end
    // end
    ...
    always_comb begin : hpd_act_evict
        ...
        if (coherence_act.send_evict) begin
            ...
            // TODO: check this if something goes wrong, might need latching
            coherence_evict_d.addr        = {st1_victim_nline, write_dir_coherence_set};
            // coherence_evict_d.addr        = {coherence_evict_rdata_q.tag, read_dir_coherence_set_q, write_dir_coherence_set};
            coherence_evict_d.is_replace  = coherence_op == OP_REPLACE;
        end
    end
    ...
    always_comb begin : hold_write_for_victim_way
        ...
        abort_inv_write                 = 1'b0;

        if (write_dir_coherence_way_d == '0 || st1_dir_victim_way == '0) begin
            if ((coherence_act_q.update_line_state && 
                write_dir_coherence_wdata.coherence_state == HPDCACHE_INVALID)) begin
                // if the line is first-time write-thru (kept invalid)
                // AND there is no available victim/MSHR, we skip the coherence update
                abort_inv_write = 1'b1;
            end
            if ((coherence_act_q.update_line_state && 
                write_dir_coherence_wdata.coherence_state == HPDCACHE_ISD)) begin
    ...
    hpdcache_tag_t wdata_tag;
    logic          wdata_tag_valid, push_tag, wtag_fifo_full, wtag_fifo_empty;

    assign push_tag = replace_read || rsp_read || fwd_read || core_req_read || refill_read;
    ...
    spill_register #(
        ...
        .clk_i   (clk_i),
        .rst_ni  (rst_ni),
        .valid_i (coherence_act.update_line_state),
        .ready_o (/* unused */),
        .data_i  (read_dir_coherence_tag_q),
        .valid_o (wdata_tag_valid),
        .ready_i (coherence_act_q.update_line_state),
        .data_o  (wdata_tag)
    );

    // fifo_v3 #(
    //   .FALL_THROUGH(1'b0),
    //   .DATA_WIDTH($bits(hpdcache_tag_t)),  // +1 for valid bit
    //   .DEPTH(2)
    // ) i_fifo_wtag (
    //   .clk_i      (clk_i),
    //   .rst_ni     (rst_ni),
    //   .flush_i    (1'b0),
    //   .testmode_i (1'b0),
    //   .full_o     (wtag_fifo_full),
    //   .empty_o    (wtag_fifo_empty),
    //   .usage_o    (/* unused */),
    //   .data_i     (read_dir_coherence_tag_d),
    //   .push_i     (push_tag && !wtag_fifo_full),
    //   .data_o     (wdata_tag),
    //   .pop_i      (coherence_act_q.update_line_state && !wtag_fifo_empty)
    // );
    ...
    always_comb begin : hpd_act_update_state
        ...
        if (coherence_act_q.update_line_state) begin
            ...
            write_dir_coherence = 1'b1;
            // write_dir_coherence = (write_dir_coherence_way_d != '0);
            // write_dir_coherence = st1_dir_victim_way != '0;
            write_dir_coherence_wdata.coherence_state       = next_coherence_state_q;
            write_dir_coherence_wdata.num_pending_inv_acks  = pending_inv_acks_q;
            // write_dir_coherence_wdata.tag                   = read_dir_coherence_tag_q;
            write_dir_coherence_wdata.tag                   = wdata_tag_valid ? wdata_tag : read_dir_coherence_tag_q;
            // write_dir_coherence_wdata.interleave_offset     = 
    ...
    always_comb begin : coherence_logic
        ...
        if (coherence_op == OP_EVICT || coherence_op == OP_REPLACE) begin
            // current_state = coherence_evict_rdata.coherence_state;
            current_state = coherence_evict_rdata_q.coherence_state;
        end else begin
            current_state = coherence_read_served_q ? read_dir_coherence_rdata.coherence_state : coherence_state_q;
        end
        ...
        unique case (current_state)
            HPDCACHE_INVALID: begin
                unique case(coherence_op)
                    ...
                    // Following cases are possible, thus must be handled
                    ...
                    OP_INV: begin
                        ...
                        coherence_act.send_inv_ack      = fwd_rx_d.need_inv_ack;
                        free_coherence                  = !fwd_rx_d.need_inv_ack;
                        coherence_state_d               = HPDCACHE_INVALID;
                    end
    ...
    hpdcache_tag_t          fwd_rx_addr_tag, evict_addr_tag;
    ...
    always_comb begin : coherence_dir_read
        ...
        //     (st1_dir_victim_tag == st2_dir_updt_tag_q) && 
        //     !evict_stall) begin
        // if (coherence_replace_d && replace_ready && !evict_stall) begin
        // if (coherence_replace && !evict_stall) begin
        ...
        // end else if (refill_core_rsp_valid_i) begin
            ...
            refill_read = 1'b1;
        end else if (fwd_rx_valid_i && fwd_rx_ready_o && (!inv_stall || fwd_rx_i.fwd_msg_type == GET)) begin
            read_dir_coherence_d = 1'b1;
            ...
            fwd_read = 1'b1;
        // FIXME: need to wait after evict is complete
        // end else if (core_req_valid_i && core_req_ready_o && !core_req_stall) begin
        end else if (pending_core_req_d && !core_req_stall) begin
            read_dir_coherence_d = 1'b1;
            // if (!coherence_busy_q) begin
            if (!coherence_busy_q || low_latency_dir_read) begin
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin : coherence_dir_read_ff
        ...
        // end else if (read_dir_coherence_d && dir_coherence_gnt && !(write_dir_coherence_q)) begin
        end else if ((read_dir_coherence_d && coherence_read_served) || coherence_op == OP_EVICT) begin
            // Read wait in case of conflict with write
            read_dir_coherence_q <= 1'b0;
    ...
    // assign write_dir_coherence_way = coherence_rway_q;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin : coherence_write_back_ff
        ...
        end else if (abort_inv_write) begin
            write_dir_coherence_q <= 1'b0;
    ...
    //  {{{
    // TODO: stall pipeline if cache dir read not served
    // logic core_req_ready_q, core_req_ready_d;
    // always_comb begin : core_req_hold
    //     st0_req_d = st0_req_q;
    //     core_req_ready_d = core_req_ready_q;
    //     if (core_req_valid_i && core_req_ready_o) begin
    //         st0_req_d = st0_req;
    //         core_req_ready_d = core_req_ready_o;
    //     end 
    // end

    // `FF(st0_req_q, st0_req_d, '0, clk_i, rst_ni)
    // // `FF(core_req_ready_q, core_req_ready_d, 1'b0, clk_i, rst_ni)
    // always_ff @(posedge clk_i or negedge rst_ni) begin
    //     if (!rst_ni) begin
    //         core_req_ready_q <= 1'b0;
    //     end else if (dir_cache_gnt) begin
    //         core_req_ready_q <= 1'b0;
    //     end else begin
    //         core_req_ready_q <= core_req_ready_d;
    //     end
    // end
    ...
    begin : st1_req_payload_ff
        ...
        // if ((core_req_ready_d | st0_rtab_pop_try_ready) && dir_cache_gnt) begin
            st1_req_q <= st0_req;
            // st1_req_q <= st0_req_q;
        end 
        // else begin
    ...
    begin : st2_metadata_ff
        ...
        if (st2_dir_updt_d) begin
            ...
            st2_dir_updt_valid_q  <= ((next_coherence_state_q != HPDCACHE_INVALID) &&
                                     (next_coherence_state_q != HPDCACHE_ISD) &&
                                     (next_coherence_state_q != HPDCACHE_IIA) &&
                                     write_dir_coherence_d) ? 1'b1 : st2_dir_updt_valid_d;
    ...
    hpdcache_memctrl #(
        ...
        .write_dir_coherence_set_i     (write_dir_coherence_set_q),       // addr (W)
        // .write_dir_coherence_way_i     (write_dir_coherence_way_d),       // cs
        .write_dir_coherence_way_i     (write_dir_coherence_way_q),       // cs
        .write_dir_coherence_wdata_i   (write_dir_coherence_wdata_d),     // wentry
        .read_dir_coherence_rdata_o    (read_dir_coherence_rdata),    // rentry
        .coherence_evict_rdata_o       (coherence_evict_rdata),
        .coherence_dir_hit_o           (coherence_dir_hit),
        .coherence_dir_bank_gnt_o      (dir_coherence_gnt),
        .cache_dir_bank_gnt_o          (dir_cache_gnt),
    ...
    logic [31:0] coherence_stall_cnt_q, coherence_stall_cnt_d;
    `FF(coherence_stall_cnt_q, coherence_stall_cnt_d, 32'h0, clk_i, rst_ni)
    always_comb begin
        coherence_stall_cnt_d   = coherence_stall_cnt_q;
        if (coherence_stall) begin
            coherence_stall_cnt_d = coherence_stall_cnt_q + 1;
        end
    end
...
`endif
    ...
    //   assert(coherence_read_served_q && write_dir_coherence_d) begin
    //     $info("[COMB DEBUG] dir read data incoming and dir write in same cycle at time %0t", $time);
    //   end else begin
    //     // No action
    //   end
    // end
